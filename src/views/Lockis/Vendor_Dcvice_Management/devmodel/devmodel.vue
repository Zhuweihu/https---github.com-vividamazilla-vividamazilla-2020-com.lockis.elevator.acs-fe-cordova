  ...
  <div class="elysExpense">
    ...
    >
      <div>
        ...
        >
          ...
          <el-table-column prop="enable" label="Enable" align="center">
          </el-table-column>
        ...
        <div>
          <el-dialog
            :title="isNew ? 'Create Devmodel' : 'Change Devmodel'"
    ...
    <div class="tishi">
      <el-dialog
        ...
        top="20%"
      >
        <span>Do you really want to delete this devmodel?</span>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        enable:[{required: true, message: 'please input enable', trigger: 'blur'}]
      },
  ...
  created: function() {
    // 总数
    this.$api.get("/miscellaneous/LksLDevmodel/count", null, r => {
    ...
    // this.$api.get("/realestate", null , r => {
  ...
  methods: {
    ...
    like(){
      ...
      if(this.likeSearch!=''&&this.likeSearch!=undefined){
        if(this.likeSearch.substr(0, 1)==' '|this.likeSearch.substr(-1)==' '){
          ...
          this.$api.get("/lksKDdev/count/"+this.likeSearch,null,r=>{
            this.totalElements = r
          })
          this.$api.get("/miscellaneous/LksLDevmodel/many/"+this.likeSearch+"/"+(this.page01-1)+'/'+this.sum, null, r => {
            this.tableData = r
          })
        }
      }else{
    ...
    createQRCode(res){
      ...
      this.designation = []
      var that =this
      this.$api.get("/LksKMachine/many/"+res.id, null, r =>{
    ...
    changemachine(){
        ...
        //   this.$api.get("/button/many/"+this.machineOptions[i].id,null,res=>{
            ...
            this._data.dialogFormVisible = false
             var uuid = this._data.form.reference
              ...
              if (uuid != ''&& uuid!=undefined ) {
                this.loading = true
                this.$api.put('/miscellaneous/LksLDevmodel/update', this._data.form, r => {
                  // history.back(-1)
                  //this.$router.back()
                  this.search()
                ...
                this.alert('aaa')
                this.$api.post('/miscellaneous/LksLDevmodel/save', this._data.form, r => {
                  //history.back(-1)
                  this.search()
                  this.realestateValue = ''
                });
    ...
    expensecreate() {
      ...
      this.form = {}
      // this._data.dialogFormVisible = true
      this.$router.push('createdevmodel')
    },
    cancel_detele() {
      // 删除弹出框取消函数
    ...
    confirm_detele() {
      ...
      this.dialogVisible = false
      this.$api.delete(
        "/miscellaneous/LksLDevmodel/" + this.dialogVisible_reference + "/purge",
        null,
        r => {
          this.$api.get("/miscellaneous/LksLDevmodel/count", null, r => {
          this.totalElements = r;
    ...
    search() {
      ...
      this.$api.get('/miscellaneous/LksLDevmodel/'+(this.page01-1)+'/'+this.sum,null,r=> {
    ...
    confirm_state() {
      ...
      // console.log(this.dialogVisible_reference)
        this.$api.delete('/LksLDevmodel/' + res + '/purge', null, r => {
          this.search();
        });
    },
    cancel_state() {
